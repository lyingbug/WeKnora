export default {
  ...
  memory: {
    ...
    settings: {
      ...
      keys: {
        'memory.enabled': { label: 'Enable long-term memory', help: 'When off, nothing is read and nothing is written. You can always switch it off for yourself.' },
        'memory.channels': { label: 'Channels', help: 'Which channels get a long-term memory space.' },
        'memory.embed_visitor_space': { label: 'Embed visitors', help: 'Visitor identity comes from the client, so persistent memory for them is opt-in.' },
        'memory.shared_space_enabled': { label: 'Shared team memory', help: 'Give the workspace a shared memory space.' },
        'memory.write.mode': { label: 'How memories are recorded', help: 'From nothing at all, through only what you explicitly ask for, to automatic extraction.' },
        'memory.write.require_review': { label: 'Ask me first', help: 'Extracted candidates wait in the review inbox until you accept them.' },
        'memory.write.allowed_types': { label: 'Types that may be recorded', help: 'For example, preferences only.' },
        'memory.write.min_confidence': { label: 'Minimum confidence', help: 'Candidates below this are discarded.' },
        'memory.write.gate_keywords': { label: 'Trigger phrases', help: 'Extraction is only considered when one of these appears.' },
        'memory.write.debounce_seconds': { label: 'Extraction debounce (s)', help: 'Several turns in one window become a single extraction.' },
        'memory.write.turn_interval': { label: 'Turn interval', help: 'Consider extraction every N turns even without a trigger phrase.' },
        'memory.write.max_notes_per_window': { label: 'Candidates per window', help: 'Upper bound on what one extraction may produce.' },
        'memory.write.extraction_model_id': { label: 'Extraction model', help: 'Leave empty to use the conversation model.' },
        'memory.recall.enabled': { label: 'Use memory in conversations', help: 'When off, memory is written but never read.' },
        'memory.recall.relevance_enabled': { label: 'Recall by relevance', help: 'When off, only always-on memories are injected and no search happens at all.' },
        'memory.recall.always_include_types': { label: 'Always-on types', help: 'These are injected on every turn.' },
        ...
        'memory.anchor.resolve_enabled': { label: 'Resolve anchors', help: 'Match entities named in a memory to knowledge-base pages.' },
        'memory.overlay.enabled': { label: 'Allow illumination', help: 'Shade the wiki graph by how much you have engaged with each page.' },
        'memory.overlay.familiar_threshold': { label: '"Familiar" threshold', help: 'Heat at which a page counts as familiar.' },
        'memory.overlay.mastered_threshold': { label: '"Mastered" threshold', help: 'Heat at which a page counts as mastered, given a confirming signal.' },
        'memory.overlay.relation_weights': { label: 'Relation weights', help: 'How much each kind of interaction contributes to heat.' },
        'memory.overlay.anchor_half_life_days': { label: 'Anchor half-life (days)', help: 'How quickly engagement fades.' },
        'memory.overlay.decay_exempt_relations': { label: 'Relations that never fade', help: 'Owning or bookmarking a subject is a standing relationship, not a recent interaction.' },
        'memory.decay.enabled': { label: 'Let memories fade', help: 'Unused memories weaken and are archived rather than accumulating forever.' },
        'memory.decay.half_life_days': { label: 'Half-life per type (days)', help: 'Preferences can last a long time; open questions should not.' },
        'memory.decay.archive_threshold': { label: 'Archive threshold', help: 'Below this strength a memory is archived. It stays visible and restorable.' },
        'memory.capacity.max_pages_per_space': { label: 'Maximum memories', help: 'Past this, the weakest are archived.' },
        'memory.retention.days': { label: 'Retention (days)', help: '0 means never purge, only archive.' },
        'memory.retention.purge_archived_after_days': { label: 'Purge archived after (days)', help: '0 keeps archived memories indefinitely.' },
        'memory.privacy.pii_redaction': { label: 'Personal information', help: 'What to do with phone numbers, email addresses and similar identifiers.' },
        'memory.privacy.blocked_patterns': { label: 'Never record', help: 'Matching content is not stored. Built-in credential patterns always apply.' },
        'memory.privacy.admin_metadata_only': { label: 'Administrators see metadata only', help: 'No role has an endpoint that returns another person\'s memory content.' },
        'memory.security.injection_sanitize': { label: 'Sanitise before injection', help: 'Strip links, code fences and instruction-shaped text before a memory reaches a prompt.' },
        'memory.security.extract_sources': { label: 'Extraction sources', help: 'Only your own messages. Never documents, search results or tool output.' },
        'memory.privacy.export_enabled': { label: 'Allow export', help: 'Let members export their own memory.' },
