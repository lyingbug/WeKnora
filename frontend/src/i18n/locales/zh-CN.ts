export default {
  ...
  memory: {
    ...
    settings: {
      ...
      keys: {
        memory_enabled: { label: '启用长期记忆', help: '关闭后不再读取也不再写入任何记忆。你随时可以为自己关闭它。' },
        ...
        memory_write_mode: { label: '写入方式', help: '从完全不记，到只记你明确要求的，再到自动抽取。' },
        memory_write_require_review: { label: '写入前先问我', help: '自动抽取的候选先进待确认列表，由你决定是否采纳。' },
        memory_write_allowed_types: { label: '允许记住的类型', help: '例如只记偏好，不记其他。' },
        ...
        memory_recall_enabled: { label: '在对话中使用记忆', help: '关闭后只写不读。' },
