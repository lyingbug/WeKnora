export default {
  ...
  memory: {
    ...
    settings: {
      ...
      keys: {
        'memory.enabled': { label: '启用长期记忆', help: '关闭后不再读取也不再写入任何记忆。你随时可以为自己关闭它。' },
        'memory.channels': { label: '生效渠道', help: '哪些渠道的用户会拥有长期记忆空间。' },
        'memory.embed_visitor_space': { label: '网页嵌入访客', help: '访客身份由客户端提供，默认不为其建立长期记忆。' },
        'memory.shared_space_enabled': { label: '团队共享记忆', help: '为工作空间建立一个共享记忆空间。' },
        'memory.write.mode': { label: '写入方式', help: '从完全不记，到只记你明确要求的，再到自动抽取。' },
        'memory.write.require_review': { label: '写入前先问我', help: '自动抽取的候选先进待确认列表，由你决定是否采纳。' },
        'memory.write.allowed_types': { label: '允许记住的类型', help: '例如只记偏好，不记其他。' },
        'memory.write.min_confidence': { label: '最低置信度', help: '低于此置信度的候选直接丢弃。' },
        'memory.write.gate_keywords': { label: '触发词', help: '出现这些说法时才考虑抽取，可按行业术语增补。' },
        'memory.write.debounce_seconds': { label: '抽取去抖（秒）', help: '一个窗口内的多轮对话合并成一次抽取。' },
        'memory.write.turn_interval': { label: '轮次间隔', help: '即使没有触发词，每 N 轮也考虑抽取一次。' },
        'memory.write.max_notes_per_window': { label: '单次候选上限', help: '一次抽取最多产生多少条候选。' },
        'memory.write.extraction_model_id': { label: '抽取模型', help: '留空则使用会话模型。' },
        'memory.recall.enabled': { label: '在对话中使用记忆', help: '关闭后只写不读。' },
        'memory.recall.relevance_enabled': { label: '按相关性召回', help: '关闭后只注入常驻记忆，连检索都不做。' },
        'memory.recall.always_include_types': { label: '常驻注入类型', help: '这些类型的记忆每轮都会注入。' },
        ...
        'memory.anchor.resolve_enabled': { label: '解析记忆锚点', help: '把记忆中提到的实体匹配到知识库页面。' },
        'memory.overlay.enabled': { label: '允许点亮知识图谱', help: '在 Wiki 图谱上按个人接触程度着色。' },
        'memory.overlay.familiar_threshold': { label: '「熟悉」阈值', help: '热度达到该值即视为熟悉。' },
        'memory.overlay.mastered_threshold': { label: '「已掌握」阈值', help: '热度达到该值且有确认信号即视为已掌握。' },
        'memory.overlay.relation_weights': { label: '关系权重', help: '不同互动方式对热度的贡献。' },
        'memory.overlay.anchor_half_life_days': { label: '锚点半衰期（天）', help: '接触热度随时间减半的周期。' },
        'memory.overlay.decay_exempt_relations': { label: '不衰减的关系', help: '负责人与收藏表达的是长期归属，不随时间变淡。' },
        'memory.decay.enabled': { label: '启用衰减', help: '长期不用的记忆逐渐减弱并归档，而不是无限堆积。' },
        'memory.decay.half_life_days': { label: '各类型半衰期（天）', help: '偏好可以很长，未解问题应该很短。' },
        'memory.decay.archive_threshold': { label: '归档阈值', help: '强度低于该值即归档（仍可查看和恢复）。' },
        'memory.capacity.max_pages_per_space': { label: '记忆条数上限', help: '超出后归档最弱的记忆。' },
        'memory.retention.days': { label: '保留期（天）', help: '0 表示不自动清除，仅归档。' },
        'memory.retention.purge_archived_after_days': { label: '归档后清除（天）', help: '0 表示永久保留归档记忆。' },
        'memory.privacy.pii_redaction': { label: '个人信息处理', help: '对手机号、邮箱等直接标识符的处理方式。' },
        'memory.privacy.blocked_patterns': { label: '禁止记录的模式', help: '命中即不落盘，内置凭证类模式始终生效。' },
        'memory.privacy.admin_metadata_only': { label: '管理员仅可见元数据', help: '任何角色都没有读取他人记忆内容的接口。' },
        'memory.security.injection_sanitize': { label: '注入前净化', help: '注入前剥离链接、代码围栏和指令式表述。' },
        'memory.security.extract_sources': { label: '抽取来源', help: '只从你本人说的话里抽取，绝不从文档或工具结果里抽取。' },
        'memory.privacy.export_enabled': { label: '允许导出', help: '允许成员导出自己的全部记忆。' },
