...
description: >
  ...
  whenever the user asks about the broader economic environment, global macro forces,
  or how traditional finance and policy decisions affect crypto and risk assets — even
  if they don't use the word "macro." Triggers include: is the macro good for crypto,
  what does the Fed mean for BTC, is this a risk-on or risk-off environment, will rate
  cuts help crypto, macro headwinds/tailwinds, Fed policy, FOMC, rate cut, rate hike,
  interest rates, inflation, CPI, PCE, jobs data, NFP, unemployment, GDP, yield curve,
  inverted yield, 10-year yield, 2-year yield, spread, DXY, dollar strength, dollar
  index, gold vs BTC, BTC vs S&P, BTC vs Nasdaq, cross-asset correlation, global
  liquidity, M2, tech earnings impact on crypto, forex, euro, yen, JPY, China macro,
  A-shares, CSI 300, economic calendar, macro environment, recession risk, soft landing,
  hard landing, liquidity cycle, global risk sentiment. Always use this skill for
  questions about the economic backdrop of crypto, central bank policy, or multi-asset
  correlation — even if phrased casually like "is the macro bad right now?"
---
...
**When the user has not specified a time range, always use the most recent data available.**

### Current date injection

The current date is available in the system prompt context (`currentDate`).
Use it to fill any endpoint that requires explicit date parameters.

For `tradfi_news(action="earnings")`:

    from_date = <currentDate>            # e.g. "2026-03-25"
    to_date   = <currentDate + 14 days>  # e.g. "2026-04-08"

Never leave `{YYYY-MM-DD}` as a literal placeholder in a real call.

### Action selection rules

| Query type | Preferred action | Avoid |
|-----------|-----------------|-------|
| Single macro indicator (CPI, NFP, PCE…) | `macro_indicators(action="latest_release", indicator=…)` | `action="history"` unless user asks for a trend over time |
| Full rates/yield snapshot | `rates_yields(action="rates_snapshot")` | `action="history"` unless user asks for trend |
| Asset price (Gold, DXY, S&P, VIX…) | `global_assets(action="price", symbol=…)` | `action="ohlcv"` unless user asks for a historical chart |
| Recent FOMC news | `macro_indicators(action="fomc_news", limit=5)` | — |
| Upcoming earnings (date window needed) | `tradfi_news(action="earnings", from_date=<today>, to_date=<today+14d>)` | hardcoded placeholder dates |

When a user asks "what is the current CPI?" or "where is gold today?", use
`latest_release` / `price`. Only switch to `history` or `ohlcv` when the user
explicitly asks for a chart, trend, or a specific past date range.
...
```
tradfi_news(action="earnings", from_date=<currentDate>, to_date=<currentDate+14d>)
# Replace <currentDate> with today's date from the system prompt context
```
