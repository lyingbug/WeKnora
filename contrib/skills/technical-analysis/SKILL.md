...
description: >
  Technical chart analysis, price indicators, and entry/exit signals for crypto assets.
  Use this skill whenever the user asks about chart patterns, price levels, indicators,
  or whether to buy/sell — even if they don't use the word "technical analysis." Always
  trigger for: is BTC overbought, what's the trend on ETH, where is support/resistance,
  give me a technical analysis, TA, chart analysis, 4h analysis, 1d setup, price target,
  entry point, stop loss level, should I buy or sell based on the chart, MACD cross, RSI
  divergence, golden cross, death cross, Bollinger Bands squeeze, breakout, breakdown,
  trend reversal, bullish signal, bearish signal, technical indicators, indicator values,
  MACD, RSI, KDJ, BOLL, SuperTrend, EMA, MA, SAR, ATR, VWAP, Fibonacci retracement,
  FIB, StochRSI, MFI, DMI, OBV, ROC, CCI, WR, CMF, Fractal, volume analysis, money
  flow, capital flow, momentum, volatility analysis. Also triggers for casual phrasing:
  "is SOL looking bullish?", "where would you enter ETH?", "what does the chart say?",
  "is this a good buy zone?", "is the trend still up?", "how's the 4h looking?". Use
  this skill for any chart-based or indicator-driven analysis question — it covers both
  single-indicator lookups and full multi-timeframe technical breakdowns.
---
...
**The Bitget kline API returns the most recent N candles when `startTime`/`endTime`
are omitted. This is the correct default — do not add date parameters unless the
user asks for a specific historical period.**

### Default fetch behavior

Correct (fetches the latest candles):

    url = 'https://api.bitget.com/api/v2/spot/market/candles?symbol={SYMBOL}&granularity={GRANULARITY}&limit={LIMIT}'

Only add `startTime`/`endTime` when the user explicitly requests a past date range.

### LIMIT defaults by timeframe

| Timeframe | Default LIMIT | Approximate coverage |
|-----------|--------------|----------------------|
| `1min`, `5min`, `15min` | 200 | ~3–33 hours |
| `30min`, `1h` | 200 | ~4–8 days |
| `4h` | 200 | ~33 days |
| `1d` | 200 | ~6–7 months |
| `1w` | 200 | ~3.8 years |

Use `limit=200` as the default for most analyses. Only reduce if the user asks for a
shorter window.

### Output timestamp

When labeling the output header, read the current date from the system prompt context
(`currentDate`) and stamp the analysis:

    ## {SYMBOL} Technical Analysis ({GRANULARITY}) · {currentDate}
