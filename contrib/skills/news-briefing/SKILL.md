...
description: >
  ...
  skill whenever the user wants to know what's happening in markets right now, catch up
  on recent developments, or understand the current narrative — even with vague or
  conversational phrasing. Always trigger for: latest news, what's happening, any news
  on X, what happened to [coin], morning briefing, today's crypto update, top headlines,
  news summary, weekly recap, what's moving markets, what's the catalyst, what are people
  talking about, breaking news, market update, trending topics, social buzz, hot search,
  Weibo trending, Chinese social media, Twitter/X crypto chatter, KOL takes, analyst
  opinions, researcher views, community narrative, current narrative, what's the story
  behind this pump/dump, why is [coin] up/down today. Also use for requests like: "give
  me a quick rundown", "catch me up", "what did I miss", "anything notable today",
  "summarize the market this week", "what's the vibe in crypto right now". This skill
  covers news + social signals — use it anytime the question is about events, narratives,
  or what's being talked about, not price charts or indicators.
---
...
**`news_feed`, `social_trending`, `tradfi_news(action="news")`, and
`tradfi_news(action="crypto_news")` all return the latest articles/posts by default
— no date parameter is required. These tools are fresh by design.**

### Output timestamp

Always include the current date in the output header so the user knows when the
briefing was compiled. Read the current date from the system prompt context (`currentDate`):

    ## News Update · {currentDate}

### Staleness caveat

RSS feeds update every 15–60 minutes. If the user asks about something that happened
"in the last hour", note: "Feed coverage may not reflect events from the past 15–60 minutes."
