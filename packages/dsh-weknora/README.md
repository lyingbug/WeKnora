...
dsh plugin --profile web add dsh-weknora
...
dsh needs a Node build with zstd support (Node ≥ 22.15 or ≥ 24) for its session persistence; the plugin itself runs on
Node ≥ 20.11.
...
Verified against dsh `0.1.0-rc.6` and WeKnora `0.7.2`. dsh is in developer preview and states that breaking changes
