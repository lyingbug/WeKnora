...
`approved_permissions` 可以省略；省略时后端会兼容旧行为，使用 manifest 中声明的 `permissions` 作为批准权限。显式提交时，批准权限的 top-level key 必须来自 manifest requested permissions。生产管理界面应展示 manifest requested permissions，并由租户管理员明确提交批准后的权限对象。
