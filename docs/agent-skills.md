...
未安装、未启用或找不到当前租户安装记录的 Skill 会被拒绝执行。`compute.memory_mb` 与 `compute.cpu` 依赖 Docker sandbox 生效，local fallback 只能提供进程级超时和基础校验。`network` 目前按 sandbox 级别强制开关执行，域名级 egress allowlist 还需要后续网络代理/网关适配后继续细化；`files`、`credentials`、`mcp` 等权限目前已能保存为批准权限，但还需要后续 sandbox/runtime 适配后继续强制。
