...
任务队列**不新增表**，直接复用 `task_pending_ops` / `task_dead_letters`（`internal/types/task_pending_op.go`），`scope='memory_space'`、`scope_id=space_id`、`dedup_key=session_id`。注意这两张表目前**缺失于 SQLite 迁移**，需先补齐，见 [5.4](#54-phase-0-的前置修复补齐-sqlite-的任务队列表)。
...
也就是说，[4.2](#42-ddl-草案) 里三处部分唯一索引（空间 owner 唯一、页 slug 唯一、锚点唯一）在 Lite 上语义不打折——这一点已在两种数据库上实测确认，见[附二](#附二设计验证记录)。
...
**约束三 · 热度与覆盖率一律在 Go 侧计算。** 仓储只负责"按 `(space_id, kb_id)` 取出锚点"这种完全可移植的查询，热度、状态判定、覆盖率聚合全部放进纯函数（`applyMemoryOverlay`、`computeCoverage`），与 `computeGraphSubset` 同构。好处有三：零方言依赖、可单测、两种形态结果**逐位一致**。把 `power()`/`ln()`/`ROLLUP` 写进 SQL 会直接锁死 Lite——[附二](#附二设计验证记录)用 Postgres 和 SQLite 两套库对照验证了这一点。
...
### 5.4 Phase 0 的前置修复：补齐 SQLite 的任务队列表

调研发现一个**既有缺陷**：`task_pending_ops` 与 `task_dead_letters` 只存在于 `migrations/versioned/000041_task_queue_and_wiki_indexes.up.sql`，`migrations/sqlite/` 里没有。也就是说全新 Lite 库上根本没有这两张表，依赖它们做持久化去抖的 **wiki ingest 在 Lite 上是残缺的**。

记忆的写路径要复用同一套模式，所以把"将这两张表补进 `migrations/sqlite/`"列为 Phase 0 的前置项。这笔改动很小，顺带把 Lite 的 wiki ingest 一起修好，属于净收益。
...
注入位置与格式约束见 [12.3](#123-记忆提示注入防护重点)。同时把本次实际注入的记忆写入 `Message.ExecutionContext`，供前端"本次使用的记忆"抽屉展示并支持点击溯源——**可解释性是这个功能能不能被信任的分水岭**。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/v1/tenants/current/memory-config` | 租户级全部设置项 + 各项取值范围与默认值（供 UI 直接渲染表单） |
| PUT | `/api/v1/tenants/current/memory-config` | 更新；服务端对隐私安全类做"只能收紧"校验，越界取值被钳制并在响应里标注 |
...
- 设置页：按 [11.5](#115-设置-ui-落点) 落三处——系统设置的租户级记忆卡片（仿会话历史 KB 卡片）、Agent 编辑页的记忆分组、个人设置的用户级项（写 `users.preferences`，无需 DDL）。表单直接由 `GET /memory/settings` 与 `GET /tenants/current/memory-config` 返回的取值范围渲染，被锁定项只读并显示锁定层级，不可用能力灰显并说明原因。新增可复用组件 `components/settings/SettingRow.vue` 承载"值 + 来源 + 锁定原因"这套三元展示。
...
| 键 | 层级 | 类型 | 默认 | 说明 |
|---|---|---|---|---|
| `memory.write.mode` | 租户 / Agent / 用户 | `off` / `explicit_only` / `gated_auto` / `always_auto` | `explicit_only` | 核心设置项：完全不写 / 仅「记住这条」与 `memory_write` 工具 / 规则门控命中才抽取 / 每个去抖窗口都抽取 |
...
| `memory.recall.show_used_memories` | 租户 / 用户 | bool | `true` | 前端"本次使用的记忆"抽屉 |
| `memory.recall.cite_memories` | 租户 / 用户 | bool | `true` | 回答中是否给记忆引用 |
...
| `memory.insights.member_coverage_visible` | 用户 | bool | `false` | 成员是否授权把个人覆盖率展示给管理员 |
| `memory.insights.auto_file_wiki_issues` | 租户 | bool | `false` | 是否自动向 `wiki_page_issues` 提 issue |
...
| 位置 | 内容 |
|---|---|
| 系统设置 → 记忆 | 租户级全部设置，仿会话历史 KB 的卡片式布局 |
| Agent 编辑页 → 记忆 | Agent 级：读写开关、注入预算、抽取模型 |
| 个人设置 → 记忆 | 用户级：总开关、写入模式、允许的记忆类型、是否需人工确认、引用展示 |
| 记忆中心右上角 | 高频项快捷入口：`write.mode`、`require_review`、`allowed_types`，以及"暂停记忆"开关 |
...
- **前置修复**：把 `task_pending_ops` / `task_dead_letters` 补进 `migrations/sqlite/`（见 [5.4](#54-phase-0-的前置修复补齐-sqlite-的任务队列表)）。这是既有缺陷，顺带修好 Lite 的 wiki ingest。
...
- 退出标准：手工创建/编辑/删除/回滚记忆页全链路通过；跨 space 与跨 tenant 的 IDOR 测试全绿；关闭时零行为变化；**同一组用例在 Postgres 与 SQLite 上结果一致**；设置项的层级合并与钳制逻辑有表驱动单测。
...
- 范围：`memory_anchors` 两份迁移；运行时 `asked_about` 锚点（零成本）；确定性锚点解析；**Go 侧纯函数** `applyMemoryOverlay` / `computeCoverage` + `overlay=memory`；覆盖率 API 与前端图例/进度条；`bridged` 图谱；`PluginMemoryBoost` 个性化召回；Wiki 页详情"你的记忆"侧栏与 `learned`/`corrected` 标记；`insights` 聚合（k-匿名）+ 自动提 `wiki_page_issues` 反哺。
...
4. **`memory.write.mode` 的出厂默认**：倾向 `explicit_only`（用户点「记住这条」才记），把自动抽取作为用户自己开启的进阶项——先建立信任再自动化。需要产品侧确认。
...
| 上游基线 | 对 v0.7.2（`18739bb1`）逐项复核本文假设 | 无假设被推翻；6 处需要跟进的上游变更已就地更新（`query_understand` 回退语义、`SearchResult.ContentRevision`/`ContentRewritten`、wiki stats 排除 `archived`、`ResolveLanguageName`、迁移号推进到 000080、自动打标签作为同类异步范式） |
| 文档引用的代码路径 | 脚本校验文档中全部代码路径 | 指向现有文件的引用全部存在；指向已删除文件的引用经 `git cat-file -e 380e371c^:<path>` 确认在删除前存在 |
| Mermaid 图 | `mermaid.parse()` | 全部通过 |
| [4.2](#42-ddl-草案) 的 Postgres DDL | `pglast` 解析 + 在 PostgreSQL 16 上实际执行（连同从 `migrations/versioned/000037` 取出的真实 `wiki_pages` 定义） | 16 条语句全部创建成功 |
| [4.2.4](#424-sqlite-版本的差异) 的 SQLite 映射 | 按文中映射表转换后在 SQLite 上实际建表 | 全部成功；**三处 `WHERE deleted_at IS NULL` 部分唯一索引在 SQLite 上语义不打折** |
| [7.2](#72-热度与状态) 热度与状态判定 | 造 6 个 Wiki 页 + 2 个记忆空间 + 8 条锚点 | 五态齐备：`mastered` / `flagged` / `familiar` / `unlit`，衰减豁免生效 |
| [7.4](#74-覆盖率) 目录下钻覆盖率 | 同一组数据 | 检索 75%、存储 50%、整体 66.7% |
| [8](#8-记忆反哺知识库) 缺口洞察 + k-匿名 | 同一组数据 | 正确识别"高频被问但内容极薄"的页，不足 k 的格子被过滤 |
| **[5.3](#53-由此产生的六条硬性设计约束) 约束三：Go 侧计算的跨库一致性** | 把热度/覆盖率实现为一个只吃"页 + 锚点"两个列表的纯函数，分别喂给 **PostgreSQL** 与 **SQLite** 取出的行（两库用完全相同的绝对时间戳种子），比对输出 | **6 行 `(slug, heat@1e-6, state, anchored_memories)` 逐位一致，覆盖率一致**。仓储侧只用了可移植 SELECT，没有 `jsonb` 运算符、`power()`、`ROLLUP`、`unnest` |
| 唯一约束语义 | 两种数据库上分别插入重复锚点 / 重复空间 owner；软删后重建 | 两边行为一致：重复被拒，软删后可重建 |

**验证中发现并已修正的设计缺陷**：`owns` 锚点在原公式下会随时间衰减，导致"我是这个主题的负责人"的页面在半年后变暗——实测里一个 400 天前的 `owns` 锚点被压到 `touched`（heat 0.050）。改为 `owns` / `bookmarked` 豁免衰减后，同一条数据变为 `familiar`（heat 0.509），符合语义。`flagged` 的判定也改为只看锚点是否存在、不看 heat。

**首轮验证方式本身也被修正**：第一版把热度与覆盖率写成了 Postgres SQL（用到 `power()`、`ln()`、`GROUP BY ROLLUP`、`unnest`），这在 Lite 上根本跑不起来。这正是 [5.3](#53-由此产生的六条硬性设计约束) 约束三存在的原因——计算必须落在 Go 侧纯函数里，仓储只做可移植的取数。

未验证、需在实现阶段确认的部分：抽取质量与成本（依赖真实 LLM 与真实会话）、读路径延迟预算、隐藏 KB 方案在大用户量下的表现、设置项层级合并在真实 UI 上的可理解性。

> 说明：上游 v0.7.2 新增的 `website-docs/scripts/check-mermaid.mjs`、`check-links.mjs` 只扫描 `website-docs/`，不覆盖 `docs/`，且没有接入 CI。本文的 Mermaid / 链接 / 路径校验是手工脚本完成的。若希望长期保障，可考虑把这类校验扩展到 `docs/` 并接入 CI——不在本规划范围内。
