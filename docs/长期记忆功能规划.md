...
| 写路径 | 规则门控、去抖入队、结构化抽取、巩固与 supersede、锚点解析，[6.2](#62-写路径门控--抽取--巩固) |
...
| 记忆中心 UI | 列表、待确认收件箱、图谱、编辑器与修订回滚，[10](#10-前端设计) |
...
| 项 | 影响 |
|---|---|
| 聊天侧：`@记忆` mention、「记住这条」按钮、「本次使用的记忆」抽屉、回答中的记忆引用 | 记忆在对话中生效但不可见；对应的 `show_used_memories` / `cite_memories` 两个设置项已**删除**，不留死开关 |
| 新会话首屏的续接建议（来自 `open_question` / `project`） | 跨 Session 的"接着上次"要靠用户自己想起 |
| Wiki 页详情的「你的记忆」侧栏与一键 `learned`/`corrected` 标记 | 只能在记忆中心手工建锚 |
...
- `WikiBrowser.vue`：图谱视图增加「点亮」开关 + 状态图例 + 覆盖率条 —— **已实现**。页面详情的"你的记忆"侧栏（锚定到本页的记忆页 + 一键标记 `learned`/`corrected`）—— **未实现**，目前只能在记忆中心手工建锚。
...
| 位置 | 层级 | 内容 |
|---|---|---|
| 系统设置 → 账户 → 长期记忆 | `user` | 本人可设的全部项，默认只显示 6 个基础项，「显示全部设置」展开其余 |
| 系统设置 → 空间 → 长期记忆策略 | `tenant` | 工作空间策略，Admin 可见（`settingsAccess.ts` 与后端 `g.Admin()` 对齐） |
| Agent 编辑页 → 对话设置 | `agent` | 该 Agent 是否读记忆、注入预算、是否写记忆；存为 `custom_agents.config.memory` 稀疏 patch |

同一个 `MemorySettingsPanel` 组件按 `level` 渲染前两处，表单结构完全由接口返回的描述符表生成，前端不硬编码任何一项设置。

记忆中心**不承载设置**，只在标题行放一个跳转到个人设置的入口。设置属于它所在的那一层：同一个值有两个家会让用户猜哪个生效。
