...
> 状态：**已实现**（Phase 0–4；Phase 5 的团队共享记忆、跨渠道身份绑定与 MCP/CLI 暴露未做）
> 基准版本：v0.7.2（`18739bb1`）
> 实现与本文的两处偏离已就地标注：召回改为 Go 侧词法打分而非隐藏向量 KB（[6.4](#64-召回的检索实现go-侧词法打分实现时的修正)），Wiki 图谱渲染器未抽取而是原地扩展（[10](#10-前端设计)）。
...
同一个 `MemorySettingsPanel` 组件按 `level` 渲染两处，表单结构完全由接口返回的描述符表生成，前端不硬编码任何一项设置：

| 位置 | 层级 | 内容 |
|---|---|---|
| 记忆中心 → 设置 | `user` | 用户可设的全部项，默认只显示 6 个基础项，「显示全部设置」展开其余 |
| 系统设置 → 记忆 | `tenant` | 工作空间策略，Admin 可见（`settingsAccess.ts` 与后端 `g.Admin()` 对齐） |

Agent 级设置（`custom_agents.config.memory`）后端已支持并参与合并，Agent 编辑页的表单尚未接，属于后续工作。
