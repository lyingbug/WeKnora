...
| 检索锚点不进桥接图谱 | `RecordRetrievalAnchors` 不写 `memory_page_id`（它确实不属于某一条记忆），而桥接图谱按记忆页筛选卫星节点 | 这是语义问题不是 bug：`asked_about` 的用途是点亮，点亮按目标聚合、工作正常。要进图谱得先定义"这条锚点属于哪条记忆" |
| 关系权重在排序里被丢弃 | `memory.overlay.relation_weights` 参与点亮，但排序 boost 对所有命中一律用 `boost.factor` | boost 默认关闭；接上要先决定权重与 factor 如何相乘 |
