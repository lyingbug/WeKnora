...
> **2026-08 重构说明：** 本文保留为第一版完整设计记录；产品主模型已收敛为“已保存记忆 + 聊天历史记忆”，详见 [长期记忆重构方案](./长期记忆重构方案.md)。图谱与知识库点亮降为可选外围能力，不再定义记忆是否成功。
...
| 点亮层 | 运行时锚点、`overlay=memory`、覆盖率、桥接图谱、匿名洞察，[7](#7-点亮层记忆--知识库) |
...
审计查出的十一条里，八条已修（见下方"已修"小节）。剩下三条留在这里，写明代价与为什么先不动。

| 项 | 影响 | 为什么先不动 |
|---|---|---|
| 洞察（insights）没有任何界面 | `GET /knowledgebase/:kb_id/memory/insights` 与其后端整条链路可用且已修正 k-匿名口径，但产品里没有入口，"哪些页被频繁问到却内容单薄"这半个价值主张够不到 | 需要一个管理员视图，是独立的一块交付 |
| `supersede` 从未发生 | `memory_pages.superseded_by` 与 `superseded` 状态是死的：矛盾语句走"覆盖正文 + 存一条修订" | 现有行为就是想要的行为（一个主题一页、历史可追溯）；删列要双库迁移且无功能收益，共享空间阶段可能真需要显式取代关系。**文档与列注释已改为陈述现状** |
| 巩固锁是进程内的 | 标准版多 worker 时同一空间可被并行巩固，且巩固路径写入一律 `expectVersion = 0` | 巩固是幂等的、归一化哈希挡住重复，实际影响有限；要修得引入跨进程锁或给巩固路径带上版本 |
| SQLite 迁移 `ADD COLUMN` 不幂等 | 迁移失败重跑会报 duplicate column，而 PG 那侧幂等 | 整套 SQLite 迁移都是无条件加列，这是既有约定；单独给一条加守卫反而不一致 |
| 图谱拖动节点会同时打开编辑器 | `GraphCanvas` 同一节点上同时绑了拖拽与点击；滚轮缩放用 `event.offsetX`，指针在节点上时视口会跳 | 交互瑕疵，不影响数据 |

#### 这一轮已修

| 项 | 原症状 |
|---|---|
| IM / API Key / Embed 渠道抽取 | 三个渠道下抽取必然失败，只留一行日志。改为把请求当时的会话 owner scope 随任务传递，worker 只在该 scope 下读会话 |
| 衰减复利 | profile 第 45 天被归档而非 999 天 |
| 写路径内容检查可绕过 | Agent 工具与编辑器直接落库，可存指令与凭据 |
| PII 身份证脱敏不全 | 保留末三位；长订单号被误伤 |
| 建锚不校验知识库可读性 | 可写进读不到的知识库，污染其覆盖率与洞察聚合 |
| 洞察 k-匿名统计错人群 | 十人读过一人反对也发布，反对者可识别 |
| 回滚静默覆盖并发编辑 | `expectVersion = 0` 关掉了乐观锁 |
| 保留期清除留下孤儿行 | 修订、锚点与已合并 note 指向不存在的页，仍在给点亮层着色 |
| 洞察 `never_lit` 无上限 | 一万页未读知识库返回一万条 |
| 桥接图谱截断提示自相矛盾 | "显示 40 / 共 25" |
| 排序 boost 匹配不到 wiki 页 | 锚点存 slug，比较的全是 id |
| 普通知识库的锚点无人消费 | 写入并计数，但四个消费方全部排除它，等于每轮写四行死数据。现由排序 boost 消费：按空间 + 本轮知识库加载提示，普通库按 knowledge id 命中、wiki 按 slug 命中 |
| 关系权重被丢弃 | 提示里算好了权重，boost 却对所有命中一律用 `boost.factor`，"纠正过"与"只提到过"加权相同 |
| 图谱标签叠成一行 | 斥力与重力在约 100px 达到平衡，而十来个汉字的标签就有这么宽 |
| 普通知识库没有点亮视图 | 点亮只存在于 wiki：叠加层画在 wiki 图谱上，覆盖率数 wiki 页，洞察过滤 wiki 页。普通知识库记录了参与度却无处可看。现已按文档实现同一套四态、衰减与覆盖率 |
| 桥接图谱只画得出一小部分锚点 | 卫星节点两道过滤——非 wiki 页排除、无 `memory_page_id` 排除——恰好把检索锚点（占绝大多数，也是普通知识库唯一的来源）全部挡在外面。于是文档列表已经点亮、图谱却是空画布 |

另有一批无引用字段与常量已清理：`MemoryHandler.writerService`、`hasEmbedder` 及其 `MemoryCapabilityReasonNoEmbeddingModel`、以及 `UpdateSpaceConfig`。仍保留但已如实标注的：`MemorySpace.VectorKBID`（隐藏向量库从未建成）、`MemoryPageRevision.EditorID`、`MemoryStats.LastUpdatedAt`、注册了但从不入队的 `TypeMemoryDecay`（衰减走 ticker）。
...
> **实现时的扩展**：本章原本只写 Wiki 知识库。点亮的计算与 Wiki 无关——热度、状态与覆盖率都是对锚点切片的纯函数，仓储投影本来就带 target kind 参数——所以**普通知识库按文档同样点亮**：同一套四态、同一套衰减、同一套覆盖率分桶（wiki 按首个面包屑，文档按 `folder_path` 首段）。
>
> 桥接图谱同样两种都画：锚点由巩固解析出来、指向某条具体记忆的，画成连线；由检索产生、属于"这个人接触过"而不属于某条记忆的，画成没有连线的卫星节点（上限 30，按参与度取）。只画第一种就等于让普通知识库永远是空画布。
>
> 两者的差别只在载体：Wiki 的叠加层折在图谱响应里（反正要画节点），文档列表没有这样的载体，所以单独有一个 `GET /knowledgebase/:kb_id/memory/overlay?target=knowledge`。洞察也接受同一个 `target` 参数，文档以存储大小充当 wiki 正文长度——更粗，但"问得多而内容少"这个信号它扛得住。
