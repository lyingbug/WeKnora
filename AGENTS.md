...
- `.env` must exist; it is gitignored, so it is NOT carried across fresh sessions. The update script bootstraps it (copies `.env.example`, forces `OLLAMA_BASE_URL=http://localhost:11434`, and ensures `SSRF_WHITELIST` includes loopback). `docker-compose*.yml` and `make dev-app` require `.env`.
