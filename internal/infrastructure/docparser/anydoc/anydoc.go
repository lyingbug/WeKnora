...
type Options struct {
	...
	Format string
	// WithAssets also extracts embedded images. It costs a second parse of
	// the same bytes, because Markdown rendering and the document model are
	// separate entry points, so callers that only index text leave it off.
	// Ignored for PDF.
	WithAssets bool
}
